...
# ==========================================================
#  System Update Manager v6.1
#  Safe Linux updater with logs, backups, snapshots,
#  rollback helpers and optional release upgrade.
#
...
for arg in "$@"; do
    case "$arg" in
        ...
        --dry-run)
            DRY_RUN=1
            ;;
        snapshot|backup|update|full|rollback|status|menu|release-upgrade|dist-upgrade)
            COMMAND="$arg"
            ;;
        --help|-h)
            ...
            echo ""
            echo "Usage:"
            echo "  sudo ./system-update-manager.sh [command] [options]"
            echo ""
            echo "Commands:"
            ...
            echo "  --yes, -y         Auto-confirm normal confirmations"
...
btrfs_rollback() {
    ...
    log "[INFO] Manual rollback depends on distro layout, subvolume names and bootloader"
    echo ""
    echo "Useful commands:"
    echo "  sudo btrfs subvolume list /"
    echo "  sudo btrfs subvolume show '$last'"
    echo ""
    echo "For production-grade Btrfs rollback use distro-native tooling:"
...
lvm_snapshot() {
    ...
    if [[ "$size_mb" -lt 1024 ]]; then
        fail "Not enough free space in VG for safe LVM snapshot. Need more free space."
    fi
...
release_upgrade() {
    ...
    case "$DISTRO" in
        ubuntu)
            ...
            echo "1) Stable upgrade, e.g. LTS -> next supported release"
            echo "2) Development upgrade, e.g. next LTS before final release"
            echo "3) Cancel"
            echo -n "> "
            read -r relopt
        ...
        fedora)
            ...
            echo -n "Target Fedora version, e.g. 41, 42, 43: "
        ...
        debian)
            log "[ERROR] Debian major upgrade is not automated by this script."
            log "[INFO] Debian release upgrade requires reviewing release notes and changing apt sources manually."
            fail "Debian release upgrade intentionally unsupported"
            ;;

        arch|endeavouros|manjaro)
            log "[INFO] Arch-based systems are rolling release."
            log "[INFO] Use normal update instead."
            ;;

        opensuse*|sles)
            log "[ERROR] openSUSE/SLES release upgrade is not automated by this script."
            log "[INFO] Use official distro documentation."
            fail "openSUSE/SLES release upgrade intentionally unsupported"
            ;;

        rhel|centos|rocky|almalinux)
            log "[ERROR] Enterprise Linux major upgrade is not automated by this script."
            log "[INFO] Use leapp or vendor-supported process."
...
full() {
    ...
    if update_system; then
        ...
        if [[ "$MODE" == "LVM" ]]; then
            log "[INFO] LVM rollback is available from menu/command"
        elif [[ "$MODE" == "BTRFS" ]]; then
            log "[INFO] Btrfs snapshot available, manual rollback required"
        else
            log "[INFO] Rsync config rollback available"
        fi
...
menu() {
    while true; do
        ...
        echo "2) Update only"
        echo "3) Full safe update"
        echo "4) Release upgrade / dist upgrade"
        echo "5) Rollback"
        echo "6) Status"
        echo "7) Exit"
        echo -n "> "
